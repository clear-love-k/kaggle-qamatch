...
# bert配置                 *mxk*
config_path = 'E:\\Pycharm_workspace\\ccf_2020_qa_match-main\\nezha\\bert_config.json'
checkpoint_path = 'E:\\Pycharm_workspace\\ccf_2020_qa_match-main\\nezha\\model.ckpt'
dict_path = 'E:\\Pycharm_workspace\\ccf_2020_qa_match-main\\nezha\\vocab.txt'
...
#                         *mxk*
path = 'E:\\Pycharm_workspace\\ccf_2020_qa_match-main\\data'

def load_data(train_test='train'):
    D = {}
    with open(os.path.join(path, train_test, train_test + '.query.tsv'),'r',encoding='UTF-8') as f:
        for l in f:
            span = l.strip().split('\t')
            D[span[0]] = {'query': span[1], 'reply': []}

    with open(os.path.join(path, train_test, train_test + '.reply.tsv'),'r',encoding='UTF-8') as f:
        for l in f:
            span = l.strip().split('\t')
            if len(span) == 4:
